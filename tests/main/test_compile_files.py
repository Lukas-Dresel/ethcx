...
def test_compile_single_file(foo_path):
    output = ethcx.compile_solidity_files([foo_path])
...
def test_compile_multiple_files(foo_path, bar_path, baz_path):
    output = ethcx.compile_solidity_files([foo_path, bar_path, baz_path])
...
def test_allow_paths(foo_path, bar_path, baz_path):
    output = ethcx.compile_solidity_files([baz_path], allow_paths=[foo_path.parent.parent])
...
def test_solc_binary(wrapper_mock, foo_path):
    ...
    ethcx.compile_solidity_files([foo_path], ["abi"], solc_binary=Path("path/to/solc"), allow_empty=True)
...
def test_solc_version(wrapper_mock, all_versions, foo_path):
    ...
    ethcx.compile_solidity_files([foo_path], ["abi"], solc_version=all_versions, allow_empty=True)
